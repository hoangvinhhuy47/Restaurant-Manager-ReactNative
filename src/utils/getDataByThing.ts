...
export default class getDataByThing {
  //format date//
  static getDayMonthYearString = date => {
    return moment(date).format("DD/MM/YYYY");//ngày/tháng/năm
  };
  static getDayMonthYearTimeString = date => {
    return moment(date).format("HH:mm DD/MM/YYYY");//ngày/tháng/năm
  };
  static getDayMonthYearHourStringDetail = date => {
    return moment(date).format("HH:mm:ss");// ngày/tháng/năm giờ:phút
  };
  static getDateTimeFormatToAPI = date => { //gọi hàm này nếu gọi api
    return moment(date).format("YYYY-MM-DDTHH:mm");
  }
  //format currency (VND)//
  static getcurrency = (currenCy) => { //hàm format tiền, khi gọi thì cộng thêm 'đ' hoặc 'VND' tùy thích 
    return currenCy.toString().split('').reverse().reduce((prev, next, index) => {
      return ((index % 3) ? next : (next + '.')) + prev
    })
  }
  //validateURL//
  static validateURL(value) { //hàm kiểm tra Url
